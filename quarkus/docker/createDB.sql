CREATE USER IF NOT EXISTS 'ssbd03*.*'@'%' IDENTIFIED BY '9LUoYTSMH';
...
GRANT ALL PRIVILEGES ON *.* TO 'ssbd03auth'@'%';
GRANT ALL PRIVILEGES ON *.* TO 'ssbd03mok'@'%';
GRANT ALL PRIVILEGES ON *.* TO 'ssbd03mow'@'%';

CREATE DATABASE IF NOT EXISTS ssbd03;

GRANT SELECT, INSERT, DELETE, UPDATE ON  *.* TO ssbd03mok;
GRANT SELECT, INSERT, DELETE, UPDATE ON  *.* TO ssbd03mok;
GRANT SELECT, INSERT, DELETE, UPDATE ON  *.* TO ssbd03mok;
GRANT SELECT, INSERT, DELETE, UPDATE ON  *.* TO ssbd03mok;
GRANT SELECT, INSERT, DELETE, UPDATE ON  *.* TO ssbd03mok;
...
GRANT SELECT ON  *.* TO ssbd03mow;
GRANT SELECT ON  *.* TO ssbd03mow;
GRANT SELECT, INSERT, UPDATE ON  *.* TO ssbd03mow;
GRANT SELECT, INSERT ON  *.* TO ssbd03mow;
GRANT SELECT, INSERT, UPDATE ON  *.* TO ssbd03mow;
GRANT SELECT, INSERT, UPDATE ON  *.* TO ssbd03mow;
GRANT SELECT, INSERT ON  *.* TO ssbd03mow;
GRANT SELECT, INSERT, UPDATE ON  *.* TO ssbd03mow;
...
--             references *.*,
--     constraint access_level_*.*_unique_constraint
--         unique (*.*_id, access_level)
-- );
-- 
-- create index *.*_*.*_id
--     on *.* (*.*_id);
-- 
-- create  *.*_confirmation_token
-- (
--     id                          bigint generated by default as identity
--         primary key,
--     creation_date_time          timestamp             not null,
...
-- );
-- 
-- create  heat_distribution_centre
-- (
--     id                          bigint generated by default as identity
--         primary key,
--     creation_date_time          timestamp not null,
...
--         constraint *.*_invalid_login_counter_check
...
-- );
-- 
-- create  heat_distribution_centre_pay_off
-- (
--     id                          bigint generated by default as identity
--         primary key,
--     creation_date_time          timestamp      not null,
...
-- 
-- create index heat_distribution_centre_pay_off_*.*_id
--     on heat_distribution_centre_pay_off (*.*_id);
-- 
-- create  *.*
-- (
...
-- 
-- create  heating_place_and_communal_area_advance
-- (
...
-- 
-- create index hot_water_entry_*.*_id
--     on hot_water_entry (*.*_id);
-- 
-- create  month_pay_off
-- (
--     id                          bigint generated by default as identity
--         primary key,
--     creation_date_time          timestamp      not null,
...
-- 
-- create index month_pay_off_*.*_id
--     on month_pay_off (*.*_id);
-- 
-- create  past_quarter_hot_water_pay_off
-- (
--     creation_date_time          timestamp      not null,
--     last_modification_date_time timestamp,
--     version                     bigint,
